...
try:
    from cyvcf2 import VCF as _Cyvcf2VCF
except Exception:
    _Cyvcf2VCF = None
...
class _SimpleVCFRecord:
    def __init__(self, chrom, pos, ref, alt, qual, info, format_keys, sample_fields, samples):
        self.CHROM = chrom
        self.POS = pos
        self.REF = ref
        self.ALT = alt
        self.QUAL = qual
        self.INFO = _VCFInfo(info)
        self._format_keys = format_keys
        self._sample_fields = sample_fields
        self.samples = samples
        self.genotypes = self._parse_genotypes()
        self.is_snp = self._is_snp()

    def _is_snp(self):
        if not self.REF or len(self.REF) != 1:
            return False
        if not self.ALT:
            return False
        for a in self.ALT:
            if not a or len(a) != 1 or a == "." or a.startswith("<"):
                return False
        return True
    ...
    def _parse_genotypes(self):
        if not self._sample_fields or not self._format_keys:
            return None
        if "GT" not in self._format_keys:
            return None
        gt_idx = self._format_keys.index("GT")
        out = []
        for sf in self._sample_fields:
            if gt_idx >= len(sf):
                out.append(None)
                continue
            gt_raw = sf[gt_idx]
            ...
            if len(parts) < 2:
                out.append(None)
                continue
            a0 = self._parse_int_or_none(parts[0])
            a1 = self._parse_int_or_none(parts[1])
    ...
    @classmethod
    def _parse_list_numbers(cls, raw):
        if raw in (None, "", "."):
            return None
        vals = []
        for x in raw.split(","):
            ...
            if x in ("", "."):
                ...
                n = cls._parse_number(x)
                vals.append(n if n is not None else None)
        return vals

    def format(self, key):
        if not self._sample_fields or not self._format_keys or key not in self._format_keys:
            return None
        idx = self._format_keys.index(key)
        out = []
        for sf in self._sample_fields:
            raw = sf[idx] if idx < len(sf) else None
            if key in ("AD", "AO", "AF"):
                out.append(self._parse_list_numbers(raw))
            elif key == "DP":
                n = self._parse_number(raw)
                out.append([n] if n is not None else None)
            elif key == "RO":
                out.append(self._parse_number(raw))
            else:
                if raw is None:
                    out.append(None)
                elif "," in raw:
                    ...
                    n = self._parse_number(raw)
                    out.append(n if n is not None else raw)
        return out
    ...
    def __iter__(self):
        with self._open() as f:
            for line in f:
                ...
                ref = parts[3]
                alt_raw = parts[4]
                alt = [] if alt_raw in (".", "") else alt_raw.split(",")
                qual_raw = parts[5]
                qual = None
                if qual_raw not in (None, "", "."):
                    try:
                        qual = float(qual_raw)
                    except Exception:
                        qual = None
                ...
                if info_raw not in (None, "", "."):
                    for token in info_raw.split(";"):
                        ...
                        if "=" in token:
                            k, v = token.split("=", 1)
                            info[k] = self._parse_info_value(k, v)
                        else:
                            info[token] = True
                ...
                if len(parts) > 8:
                    ...
                    if fmt not in (None, "", "."):
                        ...
                        for sample_raw in parts[9:]:
                            sample_fields.append(sample_raw.split(":"))

                yield _SimpleVCFRecord(
                    chrom=chrom,
                    pos=pos,
                    ref=ref,
                    alt=alt,
...
def VCF(path):
    if _Cyvcf2VCF is not None:
        return _Cyvcf2VCF(path)
    return _SimpleVCF(path)
...
# Keep HOME_DIR/input handling aligned with mapping_code.py
...
CAUSAL_CHR_LABEL = "chrx"
CAUSAL_POS = 13_500_000
...
class BulkedSegregantClassifier:
    ...
    def variants_in_interval_table(self, region, save_prefix=f"{HOME_DIR}/outputs/mapping_interval",
        ...
        # Keep labels ASCII-only to avoid mojibake in some CSV viewers.
        ...
        # Use UTF-8 BOM so Excel on Windows opens text correctly.
