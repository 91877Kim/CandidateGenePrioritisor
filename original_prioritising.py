# Auto-generated from CloudMap3_LLM-prioritization.ipynb

# ---- Cell 1 ----


# Notebook magics converted for script compatibility.
# import os
# os.chdir('/content/gdrive/MyDrive/Code/CloudMap3/mutants/hu80/')
# print(os.listdir('.'))
...
# =========================
# In notebooks this used `%pip ...`; run this once in shell if needed:
# python -m pip install --upgrade "pydantic>=2,<3" pandas python-dateutil requests tabulate
...
# Paths
# =========================
CSV_PATH = "drp5.csv"  # @param {type:"string"}
MAPPING_OUTPUT_PATH = "outputs/mapping_interval_X_0_variants.csv"  # @param {type:"string"}
...
CSV_PATH = _with_fallback(CSV_PATH, [])
MAPPING_OUTPUT_PATH = _with_fallback(MAPPING_OUTPUT_PATH, [])
...
def coalesce_flexible(colnames: List[str], df: pd.DataFrame) -> Optional[str]:
    """
    Match columns robustly by normalizing case and punctuation.
    Handles names like '#CHROM', 'chrom', 'Chromosome', etc.
    """
    ...
    for c in colnames:
        hit = norm_to_actual.get(_norm(c))
        if hit:
            return hit
    return None
...
chrom_col = coalesce_flexible(["CHROM","#CHROM","Chrom","chr","chrom","chromosome"], df)
pos_col   = coalesce_flexible(["POS","Position","POS_GRCh","pos","start"], df)
...
info_col   = coalesce_flexible(["INFO","Info","info"], df)

# Build standardized keys for primary
if chrom_col is None or pos_col is None:
    raise KeyError(
        f"Could not detect chromosome/position columns in {CSV_PATH}. "
...
# =========================
# Load hu80_mapping_output.csv (SOURCE OF truth for parental_ratio, hawaiian_ratio, QUAL, DP, EMS_label)
# =========================
...
m_chrom = coalesce_flexible(["CHROM","#CHROM","Chrom","chr","chrom","chromosome"], mout)
m_pos   = coalesce_flexible(["POS","Position","POS_GRCh","pos","start"], mout)
m_ref   = coalesce(["REF","Ref","reference_allele","ref"], mout)
m_alt   = coalesce(["ALT","Alt","alt_allele","alt"], mout)

if m_chrom is None or m_pos is None:
    raise KeyError(
        f"Could not detect chromosome/position columns in {MAPPING_OUTPUT_PATH}. "
        f"Detected columns: {list(mout.columns)}"
    )
...
# Restrict downstream analysis to mutations listed in mapping output table only.
allowed_variant_ids = set(mout["variant_id"].dropna().astype(str))
allowed_pos_keys = set(mout["pos_key"].dropna().astype(str))
print(f"[Diagnostics] Restricting downstream variants to mapping table entries: {len(allowed_pos_keys):,} unique CHROM:POS keys.")
...
for _, row in df.iterrows():
    ...
    if (pkey not in allowed_pos_keys) and (vid not in allowed_variant_ids):
        continue
...
def fetch_gene_knowledge(gene: str) -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    # PubMed refs (relevance to MCM/neuron fate if available)
    try:
        q = f'({gene}) AND (Mystery cell of male(MCM) OR neuron loss) AND elegans'
...
Context:
- MCM neurons are a pair of male-specific interneurons in the head of C. elegans. They are normally produced during larval male sexual maturation by an asymmetric division of the amphid socket glial cell (AMso). In the mutant phenotype of interest, the AMso fails to divide, so the MCM neurons are absent. We aim to identify the causal allele responsible for this defect.
...
- Prefer HIGH-impact (nonsense/frameshift/essential splice) > damaging missense > low/unknown; EMS increases prior but is not required.
...
USER_PROMPT_MAIN = {
    "task": "Prioritize variants for MCM neuron fate loss and annotate each with a narrative + confidence leveraging gene function and provided PubMed links.",
    "instructions": {
...
RANK_SYSTEM = """Rank variants for causing MCM neuron fate loss using ONLY provided summaries (fields + gene refs).
...
def llm_ranking_from_summaries(summaries: List[Dict[str, Any]]) -> LLMRankingOnly:
    user = {
        "task": "Rank all variants by likelihood of causing MCM neuron fate loss.",
...
for shot_idx in range(1, NUM_LLM_RUNS + 1):
    ...
    shot_merged_path = OUTPUT_DIR / f"original_prioritising_shot{shot_idx}_{SHOT_TIMESTAMP}.csv"
